...
- **Static vs. dynamic files:** Some downloaded code files will be static, meaning they exist on the server in the same form as they are downloaded. Some will be dynamic, meaning they are generated by the server based on varing data.

**Rendering a web page:**

1. A web page is requested
2. A DNS lookup is performed to find the location of all the addests to download for the web page
3. The assets start to be fetched (TCP handshakes, TLS negotiation, HTTP requests and responses)
4. A DOM tree is assembeled from the downloaded HTML
5. The CSSOM is built from the CSS rules
6. The JavaScript is parsed, interpreted, compiled, and executed
7. The accessibility tree is built for assistive technologies
8. The render tree is created from the DOM and CSSOM, to identify visual styles applied to each DOM node
9. Page layout is calculated
10. The styled, laid-out nodes are painted to the screen in the right order, via painting and compositing
